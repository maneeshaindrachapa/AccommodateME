...
<body style="font-family: 'Raleway', sans-serif;text-align:center">
    ...
    <h2>Input the New password we sent to you!</h2>
    <h2>to Verify your Account !</h2>
    <input type="text" name="confirmation" placeholder="New Password" class="admin-email-form">
    ...
    if(isset($_POST['confirmInput'])){
        $confirmPW=$_POST['confirmation'];
        $confirmPW_db=$crud->getData("SELECT * FROM users WHERE email='$email'");
        if($confirmPW_db[0]['password']==$confirmPW){
            if($confirmPW_db[0]['type']=='searcher'){
                header("location:process.php");
            }elseif($confirmPW_db[0]['type']=="owner"){
